...
export default function Channels() {
  ...
  async function send() {
    ...
    if (!body || !user) return
    setText('')
    const payload = { channel: CHANNEL, user_id: user.id, body }
  ...
  async function toggleReaction(message, emoji) {
    ...
    if (mine) {
      await supabase.from('message_reactions')
        .delete().match({ message_id: message.id, user_id: user.id, emoji })
    } else {
      await supabase.from('message_reactions')
  ...
  const name = (m) =>
    `${m.author_first || 'Member'}${m.author_last ? ' ' + m.author_last[0] + '.' : ''}`
...
const styles = {
  ...
  header: {
    height: '62px', flexShrink: 0, borderBottom: '1px solid #2a2a2a',
  ...
  stream: {
    flex: 1, minHeight: 0, overflowY: 'auto', padding: '22px 26px 8px',
  ...
  dayLbl: {
    ...
    letterSpacing: '1px', padding: '3px 12px', border: '0.5px solid #2a2a2a', borderRadius: '12px',
  },

  msg: { display: 'flex', gap: '12px', padding: '7px 0', position: 'relative' },
  msgReply: { padding: '4px 0' },
  avatar: {
    width: '36px', height: '36px', borderRadius: '50%', background: '#2a2a2a', color: '#ccc',
  ...
  actions: {
    position: 'absolute', top: '-6px', right: 0, display: 'flex', gap: '2px',
    background: '#161616', border: '0.5px solid #333', borderRadius: '8px', padding: '2px', zIndex: 2,
  },
  actBtn: {
    width: '28px', height: '28px', borderRadius: '6px', background: 'transparent', color: '#888',
    display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '15px', border: 'none', cursor: 'pointer',
  },
  actEmoji: {
    width: '30px', height: '28px', borderRadius: '6px', background: 'transparent', border: 'none',
  ...
  rx: {
    display: 'inline-flex', alignItems: 'center', gap: '5px', background: '#1a1a1a',
    border: '0.5px solid #333', borderRadius: '12px', padding: '3px 9px', fontSize: '11px',
    color: '#ccc', cursor: 'pointer', fontFamily: 'Montserrat, sans-serif',
  },
  rxMine: { borderColor: 'rgba(201,168,76,0.55)', background: 'rgba(201,168,76,0.10)', color: '#fff' },
  rxEmoji: { fontSize: '12px', lineHeight: 1 },

  thread: {
    margin: '6px 0 4px 48px', borderLeft: '2px solid rgba(201,168,76,0.35)', paddingLeft: '16px',
  ...
  replyBar: {
    ...
    background: '#161616', border: '0.5px solid #333', borderRadius: '8px',
    padding: '8px 8px 8px 14px', marginBottom: '8px',
  },
  ...
  composerInner: {
    ...
    border: '0.5px solid #333', borderRadius: '12px', padding: '6px 6px 6px 16px',
  },
  input: {
    flex: 1, background: 'transparent', border: 'none', outline: 'none', color: '#fff',
    fontSize: '14px', fontFamily: 'Montserrat, sans-serif', padding: '10px 0',
  },
  send: {
    width: '38px', height: '38px', borderRadius: '9px', background: '#C9A84C', color: '#0A0A0A',
    display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', border: 'none', cursor: 'pointer', flexShrink: 0,
  },
